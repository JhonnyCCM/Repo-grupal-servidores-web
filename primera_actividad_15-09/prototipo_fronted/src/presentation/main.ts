...
coachService.createCoach(
  { fullName: 'John Doe', email: 'john.doe@example.com', phone: '123456789' },
  (err, id) => {
    if (err) {
...
setTimeout(() => {
  ...
  machineService.createMachine(
    { name: 'Treadmill', description: 'Cardio machine', brand: 'Technogym', isAvailable: true },
    (err, id) => {
      if (err) {
...
setTimeout(() => {
  ...
  gymClassService.createGymClass(
    { name: 'Yoga', description: 'Relaxing yoga class', schedule: 'Mon 6pm', capacity: 15 },
    (err, id) => {
      if (err) {
